...
class RobotSdfCollisionNet():
    ...
    def _split_distribution(self, pred):
    ...
    def compute_signed_distance(self, q):
        with torch.no_grad():
            pred = self.model(q)      # no scaling if training from scratch
            mu, var, _ = self._split_distribution(pred)
            mu = mu[:, self.order]
        return mu.detach()
    
    def compute_signed_distance_distribution(self, q):
        with torch.no_grad():
            pred = self.model(q)
            mu, var, _ = self._split_distribution(pred)
            mu = mu[:, self.order]
    ...
    def compute_signed_distance_wgrad(self, q):
        ...
        if self.base_out == 1:
            ...
            with torch.enable_grad():
                ...
                m = torch.zeros((q.shape[0], 1), device=q.device)
                m[:, 0] = 1.0
            ...
            return mu.detach(), var.detach(), grads
            ...
            with torch.enable_grad():
                ...
                grads = torch.zeros(
                    (q.shape[0], q.shape[1], self.base_out),
                    device=q.device,
                    dtype=q.dtype
                )

                for i in range(self.base_out):

                    q.grad = None

                    grd = torch.zeros_like(mu)
                    grd[:, i] = 1.0

                    mu.backward(gradient=grd, retain_graph=True)

                    grads[:, :, i] = q.grad.detach()

            return mu.detach(), var.detach(), grads.detach()

    def compute_signed_distance_wgrad_original(self, q, idx = 'all'):
